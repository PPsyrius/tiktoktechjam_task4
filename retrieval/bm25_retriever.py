...
class BM25Retriever:
    def __init__(self, store, cache_dir=None, weights=DEFAULT_WEIGHTS):
        ...
        if cache_dir is None:
            ...
            self.cache_path = directory / ("fts5-v1-" + store.fingerprint + ".sqlite3")
            ...
            if not self.cache_hit:
                ...
                try:
                    ...
                    if Path(name).exists():
                        Path(name).unlink()
            self.connection = sqlite3.connect(self.cache_path.as_uri() + "?mode=ro", uri=True)
